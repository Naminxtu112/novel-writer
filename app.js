...
function renderCharacterRelationGraph(){
  ...
  relations.forEach(rel=>{
    ...
    const hit=document.createElementNS(ns,"path");hit.setAttribute("d",pathD);hit.setAttribute("fill","none");hit.setAttribute("stroke","transparent");hit.setAttribute("stroke-width","18");
    ...
    const edit=e=>{e.preventDefault();e.stopPropagation();openRelationEditor(rel,e.clientX,e.clientY)};g.addEventListener("contextmenu",edit);g.addEventListener("dblclick",edit);svg.appendChild(g);
  });
