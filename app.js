...
function downloadText(text,fileName){const b=new Blob([text],{type:"text/plain;charset=utf-8"}),a=document.createElement("a");a.href=URL.createObjectURL(b);a.download=fileName;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(a.href);}
function downloadManuscriptTxt(){downloadText(buildTxt(),`${safeFileName(currentNovel.title)}.txt`);$("exportDialog").close();}
function downloadOutlineTxt(){flushAux();downloadText(currentNovel.outlineText||"",`${safeFileName(currentNovel.title)}_アウトライン.txt`);$("exportDialog").close();}
function downloadCharactersTxt(){flushAux();downloadText(currentNovel.charactersText||"",`${safeFileName(currentNovel.title)}_キャラ設定.txt`);$("exportDialog").close();}
...
$("proofBtn").onclick=proof;$("exportBtn").onclick=()=>$("exportDialog").showModal();$("downloadTxtBtn").onclick=downloadManuscriptTxt;$("downloadOutlineTxtBtn").onclick=downloadOutlineTxt;$("downloadCharactersTxtBtn").onclick=downloadCharactersTxt;
