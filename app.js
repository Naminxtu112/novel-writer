...
// v7.6: touch support for diagram editing.
...
function mobileSelectOrActivate(el, key, activate) {
...
function attachTouchGesture(el, { key, onTap, onLongPress, holdMs = 560, moveTolerance = 10 } = {}) {
  let state = null;
  const cancelTimer = () => { if (state?.timer) clearTimeout(state.timer); if (state) state.timer = null; };
  el.addEventListener("pointerdown", e => {
    if (e.pointerType === "mouse" || e.button !== 0) return;
    state = { id:e.pointerId, x:e.clientX, y:e.clientY, moved:false, fired:false, timer:null };
  ...
  el.addEventListener("pointermove", e => {
    ...
    if (Math.hypot(e.clientX-state.x, e.clientY-state.y) > moveTolerance) { state.moved = true; cancelTimer(); }
  });
  el.addEventListener("pointerup", e => {
    if (!state || e.pointerId !== state.id) return;
    cancelTimer();
    const shouldTap = !state.moved && !state.fired;
    const fired = state.fired;
    state = null;
    if (shouldTap) {
      e.preventDefault(); e.stopPropagation();
      if (onTap) onTap(e); else if (key) mobileSelectOrActivate(el, key, null);
    }
    if (fired) setTimeout(() => { delete el.dataset.longPressed; }, 50);
  });
  el.addEventListener("pointercancel", () => { cancelTimer(); state = null; delete el.dataset.longPressed; });
}
...
function renderCharacterRelationGraph(){
  ...
  relations.forEach(rel=>{
    ...
    const dash=lineDash(rel.lineStyle);if(dash)line.setAttribute("stroke-dasharray",dash);
    if(rel.arrowStart)line.setAttribute("marker-start",`url(#rel_start_${rel.id})`);if(rel.arrowEnd)line.setAttribute("marker-end",`url(#rel_end_${rel.id})`);
    const hit=document.createElementNS(ns,"path");hit.setAttribute("d",pathD);hit.setAttribute("fill","none");hit.setAttribute("stroke","transparent");hit.setAttribute("stroke-width","30");
    const tw=Math.max(54,(rel.label||"関係").length*15+20),rb=document.createElementNS(ns,"rect");rb.setAttribute("x",labelX-tw/2);rb.setAttribute("y",labelY-15);rb.setAttribute("width",tw);rb.setAttribute("height","26");rb.setAttribute("rx","9");rb.setAttribute("fill","#fffdf8");rb.setAttribute("stroke",rel.color);
    const text=document.createElementNS(ns,"text");text.setAttribute("x",labelX);text.setAttribute("y",labelY+3);text.setAttribute("text-anchor","middle");text.setAttribute("font-size","13");text.textContent=rel.label||"関係";
    g.append(line,hit,rb,text);
    const edit=e=>{e.preventDefault();e.stopPropagation();openRelationEditor(rel,e.clientX,e.clientY)};g.addEventListener("contextmenu",edit);g.addEventListener("dblclick",edit);attachTouchGesture(hit,{key:`relation:${rel.id}`,onTap:e=>mobileSelectOrActivate(g,`relation:${rel.id}`,()=>edit(e)),onLongPress:edit});attachTouchGesture(rb,{key:`relation:${rel.id}`,onTap:e=>mobileSelectOrActivate(g,`relation:${rel.id}`,()=>edit(e)),onLongPress:edit});svg.appendChild(g);
  });
  sheets.forEach(sheet=>{const p=pos.get(sheet.id),g=document.createElementNS(ns,"g");g.classList.add("character-graph-node");g.style.cursor="grab";g.style.touchAction="none";const rect=document.createElementNS(ns,"rect");rect.setAttribute("x",p.x);rect.setAttribute("y",p.y);rect.setAttribute("width",nodeW);rect.setAttribute("height",nodeH);rect.setAttribute("rx","14");rect.setAttribute("fill","#fff");rect.setAttribute("stroke","#bfb6a8");rect.setAttribute("stroke-width","1.5");const text=document.createElementNS(ns,"text");text.setAttribute("x",p.cx);text.setAttribute("y",p.cy+5);text.setAttribute("text-anchor","middle");text.setAttribute("font-size","15");text.textContent=characterDisplayName(sheet);g.append(rect,text);const openCharacterMenu=e=>{e.preventDefault();e.stopPropagation();showFloatingPanel(e.clientX,e.clientY,panel=>{const s=document.createElement("strong");s.textContent=characterDisplayName(sheet);panel.append(s,panelButton("このキャラから関係線","primary",()=>{panel.classList.add("hidden");startRelationConnect(sheet.id)}));});};const revealCharacterCard=()=>{const card=document.querySelector(`[data-character-id="${sheet.id}"]`);if(card){card.scrollIntoView({behavior:"smooth",block:"center"});card.classList.add("flash-card");setTimeout(()=>card.classList.remove("flash-card"),900)}};g.addEventListener("click",e=>{if(relationConnectStart!==null||$("addCharacterRelationBtn").classList.contains("active-mode")){e.stopPropagation();finishRelationConnect(sheet.id)}});g.addEventListener("contextmenu",openCharacterMenu);g.addEventListener("dblclick",revealCharacterCard);attachTouchGesture(g,{key:`character:${sheet.id}`,onTap:e=>{if(relationConnectStart!==null||$("addCharacterRelationBtn").classList.contains("active-mode")){finishRelationConnect(sheet.id);return;}mobileSelectOrActivate(g,`character:${sheet.id}`,revealCharacterCard)},onLongPress:openCharacterMenu});let drag=null;g.addEventListener("pointerdown",e=>{if(e.button!==0)return;if(relationConnectStart!==null||$("addCharacterRelationBtn").classList.contains("active-mode"))return;g.setPointerCapture(e.pointerId);const pt=svgPoint(svg,e.clientX,e.clientY);drag={x:pt.x,y:pt.y,gx:sheet.graphX,gy:sheet.graphY,dx:0,dy:0};});g.addEventListener("pointermove",e=>{if(!drag)return;const pt=svgPoint(svg,e.clientX,e.clientY);drag.dx=pt.x-drag.x;drag.dy=pt.y-drag.y;g.setAttribute("transform",`translate(${drag.dx} ${drag.dy})`)});g.addEventListener("pointerup",()=>{if(g.dataset.longPressed==="1"){drag=null;g.removeAttribute("transform");return;}if(drag){const moved=Math.hypot(drag.dx,drag.dy)>3;if(moved){sheet.graphX=Math.max(nodeW/2+10,drag.gx+drag.dx);sheet.graphY=Math.max(nodeH/2+10,drag.gy+drag.dy);dirtyNovel=true;}drag=null;g.removeAttribute("transform");if(moved){renderCharacterRelationGraph();scheduleSave()}}});svg.appendChild(g);});
}
